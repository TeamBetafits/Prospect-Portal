...
import { normalizeEin, normalizeEmail, normalizeNumericCode, normalizePhone, normalizeZip } from "@/shared/forms/formatters";
...
function RadioCards({ label, options, value, onChange, columns = 2, error }: any) { return <div><div className="mb-2 text-sm font-medium text-zinc-800">{label}</div><div className={columns === 2 ? "grid gap-3 md:grid-cols-2" : "space-y-3"}>{options.map((o: string) => <label key={o} className={["flex cursor-pointer items-center gap-3 rounded-md border border-zinc-300 px-4 py-3 text-[15px]", value === o ? "border-blue-500 ring-2 ring-blue-100" : ""].join(" ")}><input type="radio" checked={value === o} onChange={() => onChange(o)} /><span>{o}</span></label>)}</div>{error ? <p className="mt-1 text-xs text-red-600">{error}</p> : null}</div>; }
function CheckboxGrid({ label, options, values, onToggle, error }: any) { return <div><div className="mb-2 text-sm font-medium text-zinc-800">{label}</div><div className="grid gap-3 md:grid-cols-2">{options.map((o: string) => <label key={o} className="flex cursor-pointer items-center gap-3 text-[15px]"><input type="checkbox" checked={values.includes(o)} onChange={() => onToggle(o)} /><span>{o}</span></label>)}</div>{error ? <p className="mt-1 text-xs text-red-600">{error}</p> : null}</div>; }
...
function CompanyPage({ v, setV, next }: any) {
  ...
  const submit = () => {
    ...
    if (v.phone && normalizePhone(v.phone) === v.phone && !/^\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}$/.test(v.phone)) e.phone = "Enter a valid 10-digit US phone number.";
    if (v.ein && !/^\d{9}$/.test(String(v.ein).replace(/\D/g, ""))) e.ein = "Use a 9-digit EIN, for example 12-3456789.";
    if (v.zipCode && !/^\d{5}(-\d{4})?$/.test(normalizeZip(v.zipCode))) e.zipCode = "Use a 5-digit ZIP code or ZIP+4.";
    if (v.preferredSicCode && !/^\d{4}$/.test(normalizeNumericCode(v.preferredSicCode))) e.preferredSicCode = "Use a 4-digit SIC code.";
    if (v.preferredNaicsCode && !/^\d{2,6}$/.test(normalizeNumericCode(v.preferredNaicsCode))) e.preferredNaicsCode = "Use a 2- to 6-digit NAICS code.";
